...
document.addEventListener("click", function (e) {
  ...
  if (cart.includes(productId)) {
    ...
    icon.setAttribute("name", "bag-add-outline");
  } else {
    cart.push(productId);
  ...
  // also update mobile nav count if present
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // ── Desktop header: WISHLIST heart button → wishlist.html
  const wishlistCounterBtn = document.getElementById("wishlist-counter");
  if (wishlistCounterBtn) {
    wishlistCounterBtn.style.cursor = "pointer";
    wishlistCounterBtn.addEventListener("click", function () {
      window.location.href = "wishlist.html";
    });
  }

  // ── Desktop header: CART bag handle button → checkout.html
  const cartCounterBtn = document.getElementById("cart-counter");
  if (cartCounterBtn) {
    cartCounterBtn.style.cursor = "pointer";
    cartCounterBtn.addEventListener("click", function () {
      window.location.href = "checkout.html";
    });
  }

  // ── "Add to cart" buttons in the featured "Deal of the day" section
  document.querySelectorAll(".add-cart-btn").forEach(btn => {
    btn.addEventListener("click", function () {
      const productIdInput = this.nextElementSibling;
  ...
  // ── Mobile bottom nav: heart icon (4th button) → wishlist.html
  const mobileNavBtns = document.querySelectorAll(".mobile-bottom-navigation .action-btn");
  if (mobileNavBtns.length >= 4) {
    mobileNavBtns[3].style.cursor = "pointer";
    mobileNavBtns[3].addEventListener("click", function () {
      window.location.href = "wishlist.html";
    });
  }

  // ── Mobile bottom nav: cart (2nd button) → checkout.html
  if (mobileNavBtns.length >= 2) {
    mobileNavBtns[1].style.cursor = "pointer";
    mobileNavBtns[1].addEventListener("click", function () {
      window.location.href = "checkout.html";
    });
  }
